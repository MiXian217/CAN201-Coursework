...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_GET:
        if FIELD_KEY not in json_data.keys():
            logger.info(f'--> Client requests list of all files for user "{username}".')
            file_dir = join('file', username)
            if os.path.exists(file_dir):
                file_list = os.listdir(file_dir)
                logger.info(f'<-- Sending list of files: {file_list}')
                connection_socket.send(
                    make_response_packet(OP_GET, 200, TYPE_FILE, f'List of files.', {'file_list': file_list}))
            else:
                logger.error(f'<-- No files directory for user "{username}".')
                connection_socket.send(
                    make_response_packet(OP_GET, 404, TYPE_FILE, f'No files directory for user "{username}".', {}))
            return
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is False and os.path.exists(
            ...
            logger.error(f'<-- The key {json_data[FIELD_KEY]} is not existing.')
            available_files = os.listdir(join('file', username))
            connection_socket.send(
                make_response_packet(OP_GET, 404, TYPE_FILE, f'The key {json_data[FIELD_KEY]} is not existing. available {available_files}', {}))
            return
    ...
    '''if request_operation == OP_UPLOAD:
        ...
        with open(file_path + '.log', 'r') as fid:
            lines = [line.strip() for line in fid.readlines() if line.strip() != '']
        rval = {
            FIELD_KEY: json_data[FIELD_KEY],
    ...
    if request_operation == OP_UPLOAD:
        ...
        # Get or create a lock for this file key
        lock = file_locks.setdefault(key, threading.Lock())

        with lock:
            # Perform file operations inside the lock context
            ...
            # Write the block data to the temp file
            try:
                with open(file_path, 'rb+') as fid:
                    fid.seek(block_size * block_index)
                    fid.write(bin_data)
            except Exception as ex:
                logger.error(f'Error writing to temp file: {ex}')
                connection_socket.send(
                    make_response_packet(OP_UPLOAD, 500, TYPE_FILE, f'Error writing to temp file: {ex}', {}))
                return

            # Write the block index to the log file
            try:
                with open(file_path + '.log', 'a') as fid:
                    fid.write(f'{block_index}\n')
                    fid.flush()
                ...
                logger.error(f'Error writing to log file: {ex}')
                connection_socket.send(
                    make_response_packet(OP_UPLOAD, 500, TYPE_FILE, f'Error writing to log file: {ex}', {}))
                return

            # Read the log file and check if all blocks have been uploaded
            try:
                with open(file_path + '.log', 'r') as fid:
                ...
                connection_socket.send(
                    make_response_packet(OP_UPLOAD, 500, TYPE_FILE, f'Error reading log file: {ex}', {}))
                return
            ...
            if len(set(lines)) == total_block:
                ...
                rval[FIELD_MD5] = md5
                os.remove(file_path + '.log')
                shutil.move(file_path, join('file', username, key))
                logger.info(f'All blocks received for key {key}. File moved to user directory.')
                # Clean up the lock
                del file_locks[key]
